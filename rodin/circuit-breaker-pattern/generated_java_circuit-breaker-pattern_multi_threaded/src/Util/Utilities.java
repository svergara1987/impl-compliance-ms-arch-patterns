...
public class Utilities {
	...
	public volatile static int[] turn; // ticket number for event i; if turn[i] == 0 then event i has no turn allocated
										// yet
	...
	public static void lock(int eid) {
		...
		for (int j = 0; j < turn.length; j++) {
			...
			while ((turn[j] != 0) && ((turn[j] < turn[eid]) || ((turn[j] == turn[eid]) && ((j < eid))))) {
			}
		}
	}
